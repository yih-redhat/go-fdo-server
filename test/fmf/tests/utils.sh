  ...
  for service in "${services[@]}"; do
    ...
    if [[ -n "${key_path}" && -n "${crt_path}" ]]; then
      ...
      if [[ -f "${sysconfig_file}" ]]; then
        sudo sed -i "s|^ADDITIONAL_OPTS=\".*\"|ADDITIONAL_OPTS=\"--server-cert-path=${dest_crt_path} --server-key-path=${dest_key_path}\"|" "${sysconfig_file}" || true
      else
        echo "ADDITIONAL_OPTS=\"--server-cert-path=${dest_crt_path} --server-key-path=${dest_key_path}\"" | sudo tee "${sysconfig_file}" >/dev/null
      fi
    fi
  done
}
